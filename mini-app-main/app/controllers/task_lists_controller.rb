...
class TaskListsController < ApplicationController
  def index
      ...
      @task_lists = Task.where(user: current_user)
       ...
       if params[:order].in? %w[new old alp]
        case params[:order]
  ...
  def destroy
    @task_list.destroy
    respond_to do |format|
      format.html { redirect_to root_url, notice: 'Task list successfully deleted!' }
  ...
  private
    ...
    def task_list_params
      params.require(:task_list).permit(:title, :description, :status)
    end
